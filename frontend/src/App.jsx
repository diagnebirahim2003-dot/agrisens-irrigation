...
function Sidebar({ auth, page, setPage, onLogout }) {
  ...
  const nav = auth.role === 'admin'
    ? [...NAV, { page:'admin', icon:'👥', label:'Utilisateurs' }]
    : NAV;
...
function BottomNav({ auth, page, setPage }) {
  const nav = auth.role === 'admin'
    ? [...NAV, { page:'admin', icon:'👥', label:'Users' }]
    : NAV;
  return (
...
function Dashboard({ auth, setPage }) {
  ...
  const cards = [
    { page:'parcelles', icon:'🧭', label:'Mes parcelles',    desc:'Gérer vos plots, cultures et stades', color:'#2e7d32', gradient:'linear-gradient(135deg,#e8f5e9,#c8e6c9)' },
    { page:'capteurs',  icon:'📡', label:'Données capteurs', desc:'Sol 8-en-1 + météo OpenWeatherMap',    color:'#1565c0', gradient:'linear-gradient(135deg,#e3f2fd,#bbdefb)' },
    { page:'calculs',   icon:'🧮', label:'Calculs ETo/ETc',  desc:'Bilan hydrique FAO-56 Penman-Monteith', color:'#e65100', gradient:'linear-gradient(135deg,#fff3e0,#ffe0b2)' },
    { page:'graphes',   icon:'📈', label:'Graphes',          desc:'Historique humidité et températures',   color:'#6a1b9a', gradient:'linear-gradient(135deg,#f3e5f5,#e1bee7)' },
  ];
  ...
  return (
    <div className="main-content">
      ...
      <div className="nav-cards">
        {cards.map(c => (
          <div key={c.page} className="nav-card"
            style={{'--nc':c.color,'--ng':c.gradient}}
